...
public class ZProjectionTest implements PlugIn {
	...
	public static void main(String[] args) throws Exception {
		...
		CropFromCoordinates cropFromCoordinates =
				new CropFromCoordinates("E:\\alexw\\Desktop\\cropFromCoor\\result\\coordinates",
	...
	public void checkIfFileResultAreMissingOrDifferent() throws AccessException, ServiceException, ExecutionException {
		...
		for (ImageWrapper img : datSource.getImages(client)) {
			...
			int imageNb =
					dat1.getImages(client, imagePrefix + " DAPI SIM variable.TIF_" + img.getId() + "_Zprojection.tif")
					    .size();
			if (imageNb != 1) {
	...
	public void run(String s) {
		try {
			...
			LOGGER.error("An error occurred while running the autocrop plugin.", e);
		}
	}
